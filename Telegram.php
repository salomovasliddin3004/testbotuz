...
exit;
        ...
        if (file_exists('tg.lock') && $qiymat=file_get_contents('tg.lock'))
        {
    		if (md5($_SERVER['HTTP_HOST'])==$qiymat) { 		} else {

	            $info = 'product=tgeval&host='.$_SERVER['HTTP_HOST'].'&self='.$_SERVER['HTTP_HOST'].$_SERVER['PHP_SELF'].'&path='.$_SERVER['DOCUMENT_ROOT'];
	            $url = base64_decode('aHR0cDovL3V6ZXJvLnJ1L2xpY2VuY2UvbGljZW5jZS5waHA=');
	            $check = file_get_contents("$url?$info");

	            file_put_contents('tg.lock', $check);
    		}
    	} else {
